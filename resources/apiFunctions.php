...
class apiFunctions {
    ...
    public function __construct($serviceId, $wsdlProvisioningPath, $wsdlAuthenticationPath, $certPath, $certPassphrase, $iwApiBaseUrl, $withErrorTrace, $withRESTResultTrace) {
        ...
        $this->authentication = new API\AuthenticationService($wsdlAuthenticationPath, array('local_cert' => $this->apiCertificate, 'passphrase' => $this->apiCertificatePassphrase));
		$this->provisioning   = new API\ConsoleAdminService($wsdlProvisioningPath, array('local_cert' => $this->apiCertificate, 'passphrase' => $this->apiCertificatePassphrase));
        $this->iwApiBaseUrl = $iwApiBaseUrl;
        $this->withErrorTrace = $withErrorTrace;
        $this->withRESTResultTrace = $withRESTResultTrace;
    }
    
    private function printError($error, $curlError = null) {
    ...
    public function AuthenticateREST($login, $code) {
        ...
        if (!$output)    {
            $this->printError($request_info, curl_errno($ch) . ": " . curl_error($ch));
            return "NOK";
        } else {
